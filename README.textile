...
Server Proxy is using web server to request data from other domain(s) and than provider to self domain,.
JavaScript needn't create a cross-domain HTTP request but just retrieve data from local host.

h3. JSONP

 SONP (JSON with padding) is a easy method, using JavaScript annotation, to share data cross domain.
JavaScript can be load from another domain but has no restriction to execution.

h3. Flash URLLoader

Flash has another security policy, it's can load form another domain by setting the policy flie crossdomain.xml.
...
h3. window.domain
domain and sub-domain can conmunicate each by setting their window's domain property to the same.
...
window.postMessage is a newer method, has less browsers support at present,  to share data between domains.
...
We can using Flash LocalConnection API to communicate cross domain, this API allow different SWF call each other, the SWF 
